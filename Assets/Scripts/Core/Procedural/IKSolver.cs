...
public class IKSolver : MonoBehaviour
{
    ...
    void Solve()
    {
        Vector3 rootPoint = bones[bones.Count - 1].bone.position;
        bones[bones.Count - 1].bone.up = -(poleTarget.position - bones[bones.Count - 1].bone.position);
        ...
        for (int i = 0; i < iterations; i++)
        {
            ...
            bones[bones.Count - 1].bone.position = rootPoint;
    ...
    private void OnDrawGizmos()
    {
        if (!ShowGizmos)
            return;
        Vector3 lastPosition = bones[bones.Count - 1].bone.position;
        for (int i = bones.Count - 2; i >= 0; i--)
        {
            Gizmos.color = Color;
            Gizmos.DrawLine(bones[i].bone.position, lastPosition);
            lastPosition = bones[i].bone.position;
        }
         Gizmos.DrawLine(lastPosition, endPointOfLastBone.position);
    }
}